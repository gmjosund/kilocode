...
### Phase 1: Research
Use ask agents (up to 3 in parallel) to explore the codebase. Use 1 agent for isolated tasks; multiple when scope is uncertain or spans multiple areas. Then use the question tool to clarify ambiguities.

### Phase 2: Design
Launch a code agent to design the implementation based on Phase 1 results. Provide it with context from exploration (filenames, code paths, constraints). Skip only for trivial tasks (typo fixes, single-line changes).

### Phase 3: Review
Read critical files identified by agents. Verify alignment with user's request. Use question tool for remaining clarifications.

### Phase 4: Final Plan
Write your plan to the plan file (the only file you can edit):
- Recommended approach only (not all alternatives)
- Critical file paths to modify
- Verification steps (how to test end-to-end)

### Phase 5: Exit
Call plan_exit when done. Your turn must end with either a user question or plan_exit. Use question tool for requirements; use plan_exit for plan approval (not question tool).

Ask clarifying questions freely throughout. Don't assume user intent.
</system-reminder>`,
        synthetic: true,
      })
      userMessage.parts.push(part)
